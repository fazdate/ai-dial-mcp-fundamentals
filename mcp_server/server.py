...
async def get_user_by_id(user_id: int) -> str:
    """Fetches user details by their user_id."""
    return await user_client.get_user(user_id)


@mcp.tool()
async def delete_user(user_id: int) -> str:
    """Removes a user from the system using their user_id."""
    return await user_client.delete_user(user_id)


@mcp.tool()
async def search_user(search_user_request: UserSearchRequest) -> str:
    """Searches for users by name, surname, email, or gender"""
    return await user_client.search_users(**search_user_request.model_dump(exclude_none=True))
...
async def update_user(user_id: int, user_update_model: UserUpdate) -> str:
    """Updates existing user information identified by user_id."""
...
async def get_flow_diagram() -> bytes:
    """Returns the flow diagram image as bytes."""
